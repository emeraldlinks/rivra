    ...
    async function loadDirPlugins(app: FastifyInstance, dir: string, prefix: string) {
    const entries = fs.readdirSync(dir, { withFileTypes: true });
    ...
    async function loadSinglePlugin(app: FastifyInstance, filePath: string, prefix?: string) {
